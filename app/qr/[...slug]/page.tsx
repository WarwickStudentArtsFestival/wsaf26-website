...
function getRedirects() {
  return qrRedirectsConfig.redirects
    .filter((redirect) => redirect.shortlink.startsWith('/qr/'))
    .map((redirect) => {
...
export function generateStaticParams() {
  ...
  return redirects.map(({ path }) => ({ slug: path.split('/').slice(1) }));
}
